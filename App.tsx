...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F9F7F2]">
      ...
      <main className="flex-1 container mx-auto px-4 py-6 max-w-4xl flex flex-col gap-6 overflow-hidden">
        {/* 模式选择器 */}
        <div className="flex bg-white/40 p-1 rounded-2xl border border-[#E8E2D6] self-center">
           <button 
            onClick={() => setActiveMode(MainMode.PRACTICE)}
            className={`px-6 py-2 rounded-xl text-xs font-bold transition-all ${activeMode === MainMode.PRACTICE ? 'bg-[#6B8E6B] text-white shadow-md' : 'text-[#8BA888]'}`}
           >
             对话练习
           </button>
           <button 
            onClick={() => setActiveMode(MainMode.DICTIONARY)}
            className={`px-6 py-2 rounded-xl text-xs font-bold transition-all ${activeMode === MainMode.DICTIONARY ? 'bg-[#6B8E6B] text-white shadow-md' : 'text-[#8BA888]'}`}
           >
             智能字典
           </button>
           <button 
            onClick={() => setActiveMode(MainMode.HISTORY)}
            className={`px-6 py-2 rounded-xl text-xs font-bold transition-all ${activeMode === MainMode.HISTORY ? 'bg-[#6B8E6B] text-white shadow-md' : 'text-[#8BA888]'}`}
           >
             学习笔记
           </button>
        </div>
            ...
            <div className="bg-[#FFFFFF]/60 backdrop-blur-sm rounded-[2.5rem] shadow-sm border border-[#E8E2D6] p-10 flex flex-col items-center justify-center relative overflow-hidden transition-all duration-700">
              ...
              <div className="text-center mt-8 z-10">
                <div className="flex items-center justify-center gap-2 mb-4">
                  <span className="flex items-center gap-1.5 px-3 py-1 bg-[#6B8E6B]/10 rounded-full text-[10px] font-bold text-[#6B8E6B] border border-[#6B8E6B]/20">
                    <div className="w-1.5 h-1.5 bg-[#6B8E6B] rounded-full animate-pulse"></div>
                    人声增强模式
                  </span>
                  <span className="px-3 py-1 bg-[#8BA888]/10 rounded-full text-[10px] font-bold text-[#8BA888] border border-[#8BA888]/20">
                    智能降噪开启
                  </span>
                </div>
                <h2 className="text-2xl font-bold text-[#4A5D4A]">
                  {status === AppStatus.IDLE && "准备好练习了吗？"}
                  {status === AppStatus.CONNECTING && "正在为您准备课堂..."}
                  {status === AppStatus.ACTIVE && "我在聆听您的声音"}
                  {status === AppStatus.ERROR && "信号出了点小问题"}
                </h2>
                <p className="text-[#8BA888] mt-2 text-sm font-medium">
                  {status === AppStatus.IDLE && "您可以放心在有环境音的地方开口说英语。"}
                  {status === AppStatus.ACTIVE && (config.showTranscription ? "文字字幕同步中..." : "纯听力模式：关注声音的每一个细节")}
                </p>
              </div>
            ...
            <div className="flex-1 flex flex-col min-h-0 bg-white/40 rounded-[2rem] border border-[#E8E2D6] overflow-hidden">
              <div className="p-4 border-b border-[#E8E2D6] bg-white/30 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {!config.showTranscription && <span className="text-[10px] font-bold px-2.5 py-1 bg-[#E8F0E8] text-[#6B8E6B] rounded-full">听力挑战中</span>}
                  {config.isCorrectionMode && <span className="text-[10px] font-bold px-2.5 py-1 bg-[#F9F1E7] text-[#C48B4D] rounded-full">精细纠错</span>}
                  <span className="text-[10px] font-bold text-[#8BA888] uppercase tracking-widest">Live API</span>
                </div>
              </div>
              <TranscriptionView 
