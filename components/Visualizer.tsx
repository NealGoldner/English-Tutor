...
const Visualizer: React.FC<VisualizerProps> = ({ status, audioContext }) => {
  ...
  useEffect(() => {
    if (status !== AppStatus.ACTIVE || !audioContext) {
    ...
    const render = () => {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        ctx.strokeStyle = `rgba(107, 142, 107, ${opacity})`;
        ctx.lineWidth = 3;
      ...
      {status === AppStatus.IDLE && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-24 h-24 bg-white/60 backdrop-blur-sm rounded-full flex items-center justify-center border-2 border-[#E8E2D6] shadow-sm">
             <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-10 h-10 text-[#6B8E6B]/40">
